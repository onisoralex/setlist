...
import type { EventDetail, EventType, Settings, SongDetail, SongSummary } from "@/lib/types";
...
const TracklistEditModal = ({ open, eventId, onDone }: TracklistEditModalProps) => {
  ...
  const load = useCallback(() => {
    Promise.all([apiFetch<EventDetail>(`/api/events/${eventId}`), apiFetch<SongSummary[]>("/api/songs")])
  ...
  const handleAdd = async (songGroupId: string) => {
    try {
      await apiFetch(`/api/events/${eventId}/songs`, {
        method: "POST",
  ...
  const handleRemove = async (trackListSongId: string) => {
    await apiFetch(`/api/events/${eventId}/songs/${trackListSongId}`, { method: "DELETE" });
    load();
  };
  ...
  const handleMove = async (index: number, direction: -1 | 1) => {
    ...
    await apiFetch(`/api/events/${eventId}/songs/reorder`, {
      method: "PATCH",
  ...
  return (
    ...
    >
      ...
      <DialogActions>
        <Button type="button" variant="contained" color="primary" onClick={onDone}>
          Done
        </Button>
      </DialogActions>
    </Dialog>
