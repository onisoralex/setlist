...
- The whole app is gated behind one shared password (`SITE_PASSWORD`), enforced in `proxy.ts` via `lib/auth.ts` — not real multi-user accounts, just a single login. Do not add further auth scaffolding "just in case." (Renamed from `middleware.ts` — Next.js 16 deprecated the `middleware` file convention in favor of `proxy`; same exported function, same `config.matcher`, no behavior change.)
...
- **MUI `palette.defaultChannel` warning**: whenever a palette color is set to a `var(--...)` string (this app's whole theming approach — see above), MUI's `cssVariables` mode can't derive an RGB "channel" token from it for alpha-blended derived colors, and warns on every render. Fixed by explicitly setting the corresponding `...Channel` field to `undefined` (MUI's own suggested fix) rather than hardcoding a literal RGB string, which would go stale the moment the underlying setting changes. Same fix applies to any other palette field set via `var()` that starts warning.
...
- **`TracklistEditModal`'s edit model**: every row action (add/remove/reorder/spacer/override) mutates local React state only — no network call per action. The whole tracklist is committed in one `PUT /api/events/:id/tracklist` transaction when the modal closes (Done or click-outside). This differs from most of the rest of the app, which is immediate-effect per action — don't add a new immediate-effect handler back into this component without re-reading `tracklist-buffer-and-batch-commit` in the Hive project's `decisions.md` first.
